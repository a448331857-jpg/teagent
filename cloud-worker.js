...
function parseFeedItems(xml, sourceName, sourceRoute) {
  const valueOf = (block, tag) => decodeXml(block.match(new RegExp(`<${tag}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/${tag}>`, "i"))?.[1] || "")
    .replace(/<[^>]+>/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return [...String(xml).matchAll(/<(?:item|entry)>([\s\S]*?)<\/(?:item|entry)>/gi)]
    .slice(0, 30)
    .map((match) => {
      const block = match[1];
      const link = valueOf(block, "link") || decodeXml(block.match(/<link[^>]+href=["']([^"']+)["']/i)?.[1] || "");
      return {
        title: valueOf(block, "title"),
        url: link,
        snippet: valueOf(block, "description") || valueOf(block, "summary") || valueOf(block, "content"),
        publishedAt: valueOf(block, "pubDate") || valueOf(block, "published") || valueOf(block, "updated"),
        sourceName,
        sourceRoute,
      };
    })
...
async function getRssRoutes(env) {
  ...
  try {
    ...
    return parsed && typeof parsed === "object" ? { ...DEFAULT_RSS_ROUTES, ...parsed } : DEFAULT_RSS_ROUTES;
  } catch {
    return DEFAULT_RSS_ROUTES;
  }
}
...
async function fetchPrivateRss(env, routeName) {
  ...
  try {
    const response = await fetch(target, {
      headers: { Accept: "application/rss+xml, application/atom+xml, application/xml, text/xml;q=0.9", "User-Agent": "TEAgent-RSS/1.0" },
...
async function handleRssApi(url, env) {
  ...
  try {
    return json(await fetchPrivateRss(env, routeName));
  } catch (error) {
    return json({ ok: false, routeName, error: error.message || "RSS 获取失败" }, 502);
  }
}

async function handleRssBundle(url, env) {
  const requested = String(url.searchParams.get("routes") || "policy,pbc,reports_10jqka,reports_eastmoney,jiemian,36kr")
  ...
  const feeds = settled.map((item, index) => item.status === "fulfilled"
    ? item.value
    : { ok: false, routeName: requested[index], error: item.reason?.message || "RSS 获取失败", items: [] });
  const items = [...new Map(feeds.flatMap((feed) => feed.items || []).map((item) => [`${item.title}|${item.url}`, item])).values()]
...
${JSON.stringify(body.context || {}).slice(0, 30000)}`;
  ...
  if (rssContext) instructions += taskType === "scheduled-research"
    ? `\n\nPublic web reference material follows. Do not mention RSS or RSSHub in the final answer. Put concrete source titles, website names and URLs in a final section named 参考来源. Treat excerpts as leads rather than verified facts and discard irrelevant material.\n\n${rssContext}`
